...
M.colorscheme_conf = {
  ...
  nord = function()
    ...
    -- Read palette from oh-my-posh (single source of truth)
    local theme_path = vim.fn.expand("~/.oh-my-posh/theme.omp.json")
    local f = io.open(theme_path, "r")
    if f then
      local p = vim.json.decode(f:read("*a")).palette
      f:close()

      vim.api.nvim_set_hl(0, "Comment", { fg = p.nord3, italic = true, bold = true })
      vim.api.nvim_set_hl(0, "FzfLuaDirPart", { fg = p.nord4 })
      vim.api.nvim_set_hl(0, "NonText", { fg = p.nord3 })
      vim.api.nvim_set_hl(0, "LineNr", { fg = p.nord3 })
    end
  end,
}
